...
def _is_bool_dtype_safe(s: pd.Series) -> bool:
    t = s.dtype
    if pd.api.types.is_bool_dtype(t):
        return True
    # Optional: handle pandas BooleanDtype without hard dependency
    return str(t).lower() == "boolean"
...
def _infer_types(df: pd.DataFrame, cfg: EDAConfig) -> Dict[str, List[str]]:
    ...
    bool_cols = [c for c in cols if pd.api.types.is_bool_dtype(df[c])]
...
STEPS: Dict[str, StepFn] = {
    ...
    "scaling_encoding": step_scaling_encoding_reco, # probably belongs to feature design -> consider removal
